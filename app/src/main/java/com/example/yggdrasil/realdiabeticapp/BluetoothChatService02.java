...
/**
 * Created by Yggdrasil on 29/3/2561.
 */
...
public class BluetoothChatService02 {
    // Debugging
    ...
    private int num_records,compare_num_rec=0;
    ...
    private String mac;
    public SparseArray<Record> resultTosend;
     ...
     */
    public BluetoothChatService02(Context context, Handler handler) {
    ...
    private void connectionFailed() {
        ...
        BluetoothChatService02.this.start();
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(BluetoothChatService.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        mHandler.sendEmptyMessage(Constants.MESSAGE_ERROR);
        bundle.putString(BluetoothChatService.TOAST, "การเชื่อมต่อกับอุปกรณ์สูญหาย กรุณาตรวจสอบการเชื่อมต่อ");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (BluetoothChatService02.this) {
                        switch (mState) {
                            case STATE_LISTEN:
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            Looper.prepare();
            try {
                ((BluetoothConnectListener)mActivity).onBluetoothConnected(BluetoothChatService02.this, mState);
            } catch (Exception e) {
    ...
    public void GetRecorddata() {
        Thread mGetRecordNumThread = new GetRecordNum();
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mGetRecordNumThread.start();
    }

    public void SyncCloseMeter() {
        Thread mCloseMeterThread = new CloseMeter();
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mCloseMeterThread.start();
    }

    public void SyncTime(byte[] time) {
        Thread mTimeSet = new TimeSet(time);
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mTimeSet.start();
    }

    public void SyncDeleteAllRecord() {
        Thread mDeleteAll = new DeleteAll();
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mDeleteAll.start();
    }

    public void SyncReadAllRecord(int rec_count) {
        Thread mReadAll = new ReadAll(rec_count);
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mReadAll.start();
    }

    public void SyncRecorddata() {
        Thread mSyncDataThread = new SyncDataThread();
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mSyncDataThread.start();
    }
    public void getNumSyncRecorddata() {
        Thread mgetNumtoSyncThread = new getNumtoSyncThread();
        //mDialogHandler.sendEmptyMessage(MSG_DIALOG_PROGRESS_SHOW);
        mgetNumtoSyncThread.start();
    }
    ...
    public class GetDataThread extends Thread {
        ...
        public void run() {
            try {
                mHandler.sendEmptyMessage(Constants.STATUS_READ_DATA);
    ...
    public class getNumtoSyncThread extends Thread {
        public getNumtoSyncThread(){
        ...
        public void run() {
            try {
                mHandler.sendEmptyMessage(Constants.STATUS_READ_DATA);
                Boolean tmp = readNumRecSyncFromMeter();
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
    ...
    public class SyncDataThread extends Thread {
        ...
        public void run() {
            try {
                // mHandler.sendEmptyMessage(ReadGluecose.STATUS_READ_DATA);
                Boolean tmp = readSyncData();
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
    ...
    public class ReadAll extends Thread {
        int rec;
        public ReadAll(int rec_count) {
            rec = rec_count;
        }
    ...
    public class GetRecordNum extends Thread {
        public GetRecordNum() {
        ...
        public void run() {
            try {
                mHandler.sendEmptyMessage(Constants.STATUS_READ_DATA);
                Boolean tmp = readNumRecFromMeter();
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
    ...
    public class CloseMeter extends Thread {
        public CloseMeter() {
    ...
    public class DeleteAll extends Thread {
        public DeleteAll() {
    ...
    public class TimeSet extends Thread {
        byte[] t = new byte[6];
        public TimeSet(byte[] time){
            t = time;
        }
        public void run(){
            try{
                mHandler.sendEmptyMessage(Constants.STATUS_READ_DATA);
                Boolean tmp = timeSet(t);
            } catch (Exception e){
                e.printStackTrace();
            }
        }
    ...
    public SparseArray<Record> getResult(){
        return resultTosend;

    }
    public void setResult(SparseArray<Record> send){
        resultTosend = send;
    }
    ...
    private byte[] readFromMeter(int paramInt) throws Exception
    {
        ...
        for (i = 0; i < paramInt; ++i)
        {
            ...
            Log.d("Byte[" + String.valueOf(i) +"], ", String.valueOf(j));
        }
    ...
    public boolean readDataFromMeter() throws Exception {
        ...
        sc_sendBuf2[4] = (byte)compare_num_rec;//data
        ...
        msg = mHandler.obtainMessage(BluetoothChatService.MESSAGE_RESULT);
        Log.d(TAG, "Data: "+record.getYear()+" ,"+record.getMon()+" ,"+record.getRealDay()+" ,"+record.getResult()+" ,"+record.getEvent()+" ,"+record.getHour()+" ,"+record.getMin());
        bundle = new Bundle();
        bundle.putString("result", record.toString());
        bundle.putString("date", record.getDay());
        bundle.putString("time", record.getTimeText());
        bundle.putInt("hour",record.getHour());
        bundle.putInt("min",record.getMin());
        bundle.putInt("day",record.getRealDay());
        bundle.putInt("month",record.getMon());
        bundle.putInt("year",record.getYear());
        //bundle.putInt("num_rec",recordsCount);
        bundle.putInt("bloodValue",record.getResult());
        bundle.putInt("event",record.getEvent());
    ...
    public boolean sendCloseMeter() throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;
        ...
        Log.d(TAG, "Data = " + sc_receiveBuf1[0] + sc_receiveBuf1[1] + sc_receiveBuf1[2] + sc_receiveBuf1[3]
                + sc_receiveBuf1[4] + sc_receiveBuf1[5]);
        //int total = num_records-recordsCount;
        msg = mHandler.obtainMessage(Constants.MESSAGE_CLOSE);
        bundle = new Bundle();
        //bundle.putInt("num_rec", total);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    public boolean readAll(int recCount) throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;
        ...
        for( int i=1; i <= recCount; i++) {
            ...
            if( i == 1){
                msg = mHandler.obtainMessage(Constants.MESSAGE_READ_START);
                mHandler.sendMessage(msg);
            }
            byte sc_sendBuf1[] = new byte[7];
            byte sc_receiveBuf1[];
            // get the number of records
            sc_sendBuf1[0] = STX;//stx
            sc_sendBuf1[1] = (byte) 0x02;//size
            sc_sendBuf1[2] = (byte) ~(sc_sendBuf1[1]);//~size
            sc_sendBuf1[3] = (byte) 0x01;//command

            //sc_sendBuf1[4] = (byte) 0x00;    // data
            sc_sendBuf1[4] = (Byte.parseByte(String.valueOf(recCount-i)) );

            sc_sendBuf1[5] = (byte) ~(sc_sendBuf1[0] ^ sc_sendBuf1[2] ^ sc_sendBuf1[4]); //checksum L
            sc_sendBuf1[6] = (byte) ~(sc_sendBuf1[1] ^ sc_sendBuf1[3]); // checksum H
            try {
                sc_receiveBuf1 = dialogWithMeter(sc_sendBuf1, 13);
            } catch (Exception e) {
                closeConnection();
                //Logger.printException(e);
                return false;
            ...
            while(start_data <=10 ){
                data[start_data - 5] = sc_receiveBuf1[start_data];
                Log.d("Test for byte [" + (start_data - 6) + "]", String.valueOf(sc_receiveBuf1[start_data] ) );
                start_data += 1;
            }
            Log.d(TAG, "Data = " + sc_receiveBuf1[0] + sc_receiveBuf1[1] + sc_receiveBuf1[2] + sc_receiveBuf1[3]
                    + sc_receiveBuf1[4] + sc_receiveBuf1[5] + sc_sendBuf1[6]);
            //int total = num_records-recordsCount;
            msg = mHandler.obtainMessage(Constants.MESSAGE_READ_CONTINUE);
            bundle = new Bundle();
            bundle.putByteArray("value_byte[]", data);
            msg.setData(bundle);
            mHandler.sendMessage(msg);
        ...
        mHandler.sendEmptyMessage(Constants.MESSAGE_READ_END);
        return true;
    }
    ...
    public boolean deleteAll() throws Exception {
        ...
        sc_sendBuf1[1] = (byte)0x01;//size
        sc_sendBuf1[2] = (byte) ~ ( sc_sendBuf1[1] );//~size
        sc_sendBuf1[3] = (byte)0x14;//command
        sc_sendBuf1[4] = (byte) ~(sc_sendBuf1[0] ^ sc_sendBuf1[2]); //checksum L
        sc_sendBuf1[5] = (byte) ~(sc_sendBuf1[1] ^ sc_sendBuf1[3]); // checksum H
        try{
            sc_receiveBuf1 = dialogWithMeter(sc_sendBuf1, 6);
        ...
        Log.d(TAG, "Data = " + sc_receiveBuf1[0] + sc_receiveBuf1[1] + sc_receiveBuf1[2] + sc_receiveBuf1[3]
                + sc_receiveBuf1[4] + sc_receiveBuf1[5]);
        //int total = num_records-recordsCount;
        msg = mHandler.obtainMessage(Constants.MESSAGE_RECORD_DELETE_ALL);
        bundle = new Bundle();
        //bundle.putInt("num_rec", total);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    public boolean timeSet(byte[] time) throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;

        byte  sc_sendBuf1[] = new byte[12];
        byte  sc_receiveBuf1[] = new byte[12];

        // get the number of records
        sc_sendBuf1[0] = STX;//stx
        sc_sendBuf1[1] = (byte)0x07;//size
        sc_sendBuf1[2] = (byte) ~ ( sc_sendBuf1[1] );//~size
        sc_sendBuf1[3] = (byte)0x04;//command
        sc_sendBuf1[4] = time[0];
        sc_sendBuf1[5] = time[1];
        sc_sendBuf1[6] = time[2];
        sc_sendBuf1[7] = time[3];
        sc_sendBuf1[8] = time[4];
        sc_sendBuf1[9] = time[5];

        sc_sendBuf1[10] = (byte) ~(sc_sendBuf1[0] ^ sc_sendBuf1[2] ^
                sc_sendBuf1[4] ^ sc_sendBuf1[6] ^ sc_sendBuf1[8]); //checksum L
        sc_sendBuf1[11] = (byte) ~(sc_sendBuf1[1] ^ sc_sendBuf1[3]
                ^ sc_sendBuf1[5] ^ sc_sendBuf1[7] ^ sc_sendBuf1[9]); // checksum H
        Log.d(TAG, "Data = " + sc_sendBuf1[0]+"\n" + sc_sendBuf1[1]+"\n" +
                sc_sendBuf1[2]+"\n" + sc_sendBuf1[3]+"\n"
                + sc_sendBuf1[4]+"\n" + sc_sendBuf1[5]+"\n" + sc_sendBuf1[6]+"\n" + sc_sendBuf1[7]+"\n"
                + sc_sendBuf1[8]+"\n" + sc_sendBuf1[9]+"\n" + sc_sendBuf1[10]+"\n" + sc_sendBuf1[11]);
        Log.d(TAG, String.valueOf(sc_sendBuf1[0] + "\n" + sc_sendBuf1[1] + "\n" +
                sc_sendBuf1[2] + "\n" + sc_sendBuf1[3] + "\n" + sc_sendBuf1[4] + "\n" +
                sc_sendBuf1[5] + "\n" + sc_sendBuf1[6] + "\n" + sc_sendBuf1[7] + "\n" +
                sc_sendBuf1[8] + "\n" + sc_sendBuf1[9] + "\n" + sc_sendBuf1[10] + "\n" +
                sc_sendBuf1[11] + "\n" ));
        try{
            sc_receiveBuf1 = dialogWithMeter(sc_sendBuf1, 12);
        }
        catch(Exception e)
        {
            closeConnection();
        ...
        Log.d(TAG, "Data = " + sc_receiveBuf1[0] + sc_receiveBuf1[1] + sc_receiveBuf1[2] + sc_receiveBuf1[3]
                + sc_receiveBuf1[4] + sc_receiveBuf1[5] + sc_receiveBuf1[6] + sc_receiveBuf1[7]
                + sc_receiveBuf1[8] + sc_receiveBuf1[8] + sc_receiveBuf1[10] + sc_receiveBuf1[11]);
        //int total = num_records-recordsCount;
        msg = mHandler.obtainMessage(Constants.MESSAGE_CLOSE);
        bundle = new Bundle();
        //bundle.putInt("num_rec", total);
    ...
    public boolean readNumRecFromMeter() throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;
        ...
        //int total = num_records-recordsCount;
        msg = mHandler.obtainMessage(Constants.MESSAGE_RECORD_CONFIRM);
        bundle = new Bundle();
        //bundle.putInt("num_rec", total);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    public boolean readNumRecSyncFromMeter() throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;
        ...
        Log.d(TAG, "Data = " + sc_sendBuf1[0]+"\n" + sc_sendBuf1[1]+"\n" +
                sc_sendBuf1[2]+"\n" + sc_sendBuf1[3]+"\n"
                + sc_sendBuf1[4]+"\n" + sc_sendBuf1[5]);
        Log.d(TAG, String.valueOf(sc_sendBuf1[0]) + "\n" + String.valueOf(sc_sendBuf1[1]) + "\n" +
                String.valueOf(sc_sendBuf1[2]) + "\n" + String.valueOf(sc_sendBuf1[3]) + "\n" + String.valueOf(sc_sendBuf1[4]) + "\n" +
                String.valueOf(sc_sendBuf1[5]) );
        try{
            sc_receiveBuf1 = dialogWithMeter(sc_sendBuf1, 7);
        }
        catch(Exception e)
        ...
        Log.d(TAG, "NUM_REC = " + num_records);
        //int total = num_records-recordsCount;
        msg = mHandler.obtainMessage(Constants.MESSAGE_RECORD_COUNT);
        bundle = new Bundle();
        bundle.putInt("num_rec", sc_receiveBuf1[4]);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    public boolean readSyncData() throws Exception {
        Message msg;
        Bundle bundle;
        Record record;
        Record lastRecord = null;
        SparseArray<Record> groupRec= new SparseArray<Record>();
        int i=0;
        Log.d(TAG,"NUM_REC = " + compare_num_rec);
        msg = mHandler.obtainMessage(Constants.MESSAGE_PROGRESS_MAX);
        bundle = new Bundle();
        bundle.putInt("progressMax", compare_num_rec);
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        ...
        while(compare_num_rec>=0){
            ...
            record = new Record(sc_receiveBuf2, 5);
            groupRec.append(i,record);
            i++;
            compare_num_rec--;
            mHandler.sendEmptyMessage(Constants.MESSAGE_PROGRESS);

            Thread.sleep(200);
        }
        setResult(groupRec);
        mHandler.sendEmptyMessage(Constants.MESSAGE_SYNC_COMPLETE);
    ...
    public boolean setUnits(int units){
        ...
        try {
            ...
            Message msg = mHandler.obtainMessage(BluetoothChatService.MESSAGE_TOAST);
            Bundle bundle = new Bundle();
            bundle.putString(BluetoothChatService.TOAST, "Units set successfully");
            msg.setData(bundle);
            mHandler.sendMessage(msg);
            return true;
        } catch (Exception e) {
